...
crate_type = CARGO_PROJECT_TYPE);
  ...
  let cargo_dir = out_dir.join(&hidden_cargo_dir_name);
  ...
  let depot_deps = out_dir.join("deps");
  let depot_native = out_dir.join("native");
  ...
  if cargo_toml_path.exists() && cargo_toml_path.is_file() {
    ...
    fs::rename(out_dir.join(&cargo_project_name), out_dir.join(&hidden_cargo_dir_name)).unwrap();
